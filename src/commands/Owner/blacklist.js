    ...
    if(interaction.guild.ownerId !== interaction.member.id) return interaction.reply({ content: `\`[❗]\` ${interaction.member}, vous n'avez pas la permission d'utiliser cette commande.` })
        ...
        switch(type) {
            case 'add': {
                ...
                con.query(`SELECT * FROM blacklist WHERE userID = '${target.id}'`, function(err, result) {
                    if(!result[0]) {
                        con.query(`INSERT INTO blacklist (userID, reason) VALUES ('${target.id}', '${reason}')`, function(err, result) {
                            return interaction.reply({
                                embeds: [{
                                    color: Colors.Blue,
                                    description: `${target} vient d'être ajouté(e) à la liste noire.`
                                }]
                            }).then(async () => {
                        ...
                        return interaction.reply({
                            embeds: [{
                                color: Colors.Blue,
                                description: `${target} est déjà dans la liste noire.`
                            }]
                        })
                    }
                })
            ...
            case 'remove': {
                ...
                con.query(`SELECT * FROM blacklist WHERE userID = '${target.id}'`, function(err, result) {
                    if(!result[0]) return interaction.reply({
                        embeds: [{
                            color: Colors.Blue,
                            description: `${target} n'est pas dans la liste noire.`
                        }]
                    });
    
                    con.query(`DELETE FROM blacklist WHERE userID = '${target.id}'`, function(err, result) {
                        return interaction.reply({
                            embeds: [{
                                color: Colors.Blue,
                                description: `${target} vient d'être retiré(e) de la liste noire.`
                            }]
                        })
                    })
                })
            ...
            case 'list': {
                con.query(`SELECT * FROM blacklist ORDER BY createdAt DESC`, function(err, result) {
                    const pageSize = 10;
                    let currentPage = 0;
